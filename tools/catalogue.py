...
def build():
    ...
    return {
        ...
        "source": {
            ...
            "catalogue": f"{RELEASE}/catalogue.json",
            "downloads": RELEASE,
        },
        "install": {
            "format": "zip",
...
if __name__ == "__main__":
    ...
    if "--print" in sys.argv:
        ...
        # dist/ only, never committed. The catalogue records the byte size of
        # each archive, and a zip is not identical across zlib versions -- so a
        # committed copy drifts from the one CI builds for reasons nobody can
        # act on. It travels with the archives it describes instead, in the same
        # release, where the two cannot disagree.
